...
- [x] **소유 범위(`OfficeArea`) 도입**
  - 지금까지 "내 회사 범위"라는 개념이 코드에 아예 없었다. `CheckTile()`은 빨간 타일만 봤기 때문에
    벽 밖은 물론 **바닥 밖 허공에도** 가구를 놓을 수 있었다
  - 씬도 구조가 맞지 않았다. 벽 2개가 감싸는 방은 20x20 유닛(10x10칸)인데
    바닥은 40x40 유닛(20x20칸)이라 **바닥이 방보다 4배 넓게** 사방으로 5칸씩 삐져나와 있었다.
    화면 대부분이 정체불명의 흰 평면이라 허전해 보이던 원인
  - `OfficeArea`가 값 하나(Land/Office)를 들고 바닥 크기, 벽 2개의 위치·길이,
    배치 가능 범위, 잠긴 타일을 전부 계산한다. 씬에서 벽을 손으로 옮길 일이 없다
  - **Land** = 바닥 전체 = 최대 확장 범위 / **Office** = 지금 소유한 범위 / **Locked** = 그 차이
  - `PlaceSystem`과 같은 오브젝트에 붙고 `GetComponent`로 참조한다 (`LongPressSelector`와 같은 규칙).
    컴포넌트가 없으면 예전처럼 범위 제한 없이 동작한다
- [x] **`CheckTile()` 경계 검사**
  - 발자국이 통째로 Office 안에 들어가야 배치된다. 빨간 타일 검사만으로는 못 막는다 —
    사무실 밖은 아무것도 안 놓여있어서 타일이 비어있기 때문
  - `TakenArea()`도 사무실 밖 칸을 빨갛게 칠한다. ok를 누르기 전에 왜 안 놓이는지 보여야 한다
- [x] **미개방 구역 표시**
  - Land - Office를 어두운 반투명 타일(`lockedtile.asset`)로 덮는다.
    "여기까지 넓힐 수 있다"가 보여야 확장 욕구가 생긴다 (Two Point Hospital, 심즈 건축모드 방식)
  - **배치용 타일맵과 반드시 분리해야 한다.** `SetArea()`가 매 프레임 `mainTilemap`을
    지웠다 다시 칠하기 때문에, 같이 쓰면 잠긴 타일이 같이 지워진다.
    그래서 `Tilemap_Locked`를 따로 두고 `SortingOrder: -1`로 배치 타일 아래에 깐다
- [ ] **확장 버튼 / 서버 저장**
  - `OfficeArea.Expand(Vector2Int)`까지는 있다. 이걸 부르는 UI와, 늘린 크기를 서버에 저장하는 경로가 없다
  - 저장할 때 Office의 origin/size를 같이 넣어야 한다. 안 넣으면 재접속 시 사무실이 초기 크기로 돌아가고
    벽 밖에 있던 가구가 배치 불가 영역에 갇힌다
